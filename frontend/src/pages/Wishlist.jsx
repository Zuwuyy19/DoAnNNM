...
export default function Wishlist() {
  ...
  const enrolledSet = useMemo(
    () => new Set(enrolledCourseIds.map((id) => id?.toString())),
    [enrolledCourseIds]
  );
  ...
  const handleAddToCart = (e, course) => {
    ...
    if (enrolledSet.has(course._id?.toString())) {
  ...
  const fetchWishlist = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        setEnrolledCourseIds((profile.enrolledCourses || []).map((course) => course._id));
      }
    } catch (err) {
      ...
      <div className="course-detail-inner">
        ...
        {wishlist.length === 0 ? (
          ...
          <div className="courses-grid">
            {wishlist.map((course) => {
              const isOwned =
                user?.role === "admin" || enrolledSet.has(course._id?.toString());

              return (
                <div className="course-card" key={course._id}>
                  <Link to={isOwned ? `/learning/${course.slug}` : `/courses/${course.slug}`}>
                    <img
                      src={course.image}
                      alt={course.title}
                  ...
                  <div className="course-content">
                    ...
                    <div className="course-footer">
                      <span className="course-price">
                        {isOwned ? (
                          <span className="price-free" style={{ color: "var(--success)" }}>
                            Đã sở hữu
                          </span>
                        ) : course.price === 0 ? (
                          ...
                          <span className="price-normal">{course.price.toLocaleString("vi-VN")}đ</span>
                        )}
                      </span>
